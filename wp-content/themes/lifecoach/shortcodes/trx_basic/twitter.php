...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('lifecoach_sc_twitter_theme_setup')) {
	add_action( 'lifecoach_action_before_init_theme', 'lifecoach_sc_twitter_theme_setup' );
	function lifecoach_sc_twitter_theme_setup() {
		add_action('lifecoach_action_shortcodes_list', 		'lifecoach_sc_twitter_reg_shortcodes');
		if (function_exists('lifecoach_exists_visual_composer') && lifecoach_exists_visual_composer())
			add_action('lifecoach_action_shortcodes_list_vc','lifecoach_sc_twitter_reg_shortcodes_vc');
	}
}
...
if (!function_exists('lifecoach_sc_twitter')) {	
	function lifecoach_sc_twitter($atts, $content=null){	
		...
		$twitter_username = $user ? $user : lifecoach_get_theme_option('twitter_username');
		$twitter_consumer_key = $consumer_key ? $consumer_key : lifecoach_get_theme_option('twitter_consumer_key');
		$twitter_consumer_secret = $consumer_secret ? $consumer_secret : lifecoach_get_theme_option('twitter_consumer_secret');
		$twitter_token_key = $token_key ? $token_key : lifecoach_get_theme_option('twitter_token_key');
		$twitter_token_secret = $token_secret ? $token_secret : lifecoach_get_theme_option('twitter_token_secret');
		$twitter_count = max(1, $count ? $count : intval(lifecoach_get_theme_option('twitter_count')));
		...
		if (!empty($twitter_consumer_key) && !empty($twitter_consumer_secret) && !empty($twitter_token_key) && !empty($twitter_token_secret)) {
			...
			if ($data && isset($data[0]['text'])) {
				lifecoach_enqueue_slider('swiper');
				$output = ($bg_color!='' || $bg_image!='' || $bg_overlay>0 || $bg_texture>0 || lifecoach_strlen($bg_texture)>2 || ($scheme && !lifecoach_param_is_off($scheme) && !lifecoach_param_is_inherit($scheme))
						? '<div class="sc_twitter_wrap sc_section'
								...
								. ($align && $align!='none' && !lifecoach_param_is_inherit($align) ? ' align' . esc_attr($align) : '')
								. '"'
							.' style="'
						...
						. '<div class="sc_twitter'
								...
								. ($bg_color=='' && $bg_image=='' && $bg_overlay==0 && ($bg_texture=='' || $bg_texture=='0') && $align && $align!='none' && !lifecoach_param_is_inherit($align) ? ' align' . esc_attr($align) : '')
								. '"'
							...
							$output .= '<div class="swiper-slide" data-style="'.esc_attr(($ws).($hs)).'" style="'.esc_attr(($ws).($hs)).'">'
										. '<div class="sc_twitter_item">'
											...
											. '<div class="sc_twitter_content">'
												...
												. force_balance_tags(lifecoach_prepare_twitter_text($tweet))
											. '</div>'
										. '</div>'
									. '</div>';
		...
		return apply_filters('lifecoach_shortcode_output', $output, 'trx_twitter', $atts, $content);
	}
	lifecoach_require_shortcode('trx_twitter', 'lifecoach_sc_twitter');
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'lifecoach_sc_twitter_reg_shortcodes' ) ) {
	//add_action('lifecoach_action_shortcodes_list', 'lifecoach_sc_twitter_reg_shortcodes');
	function lifecoach_sc_twitter_reg_shortcodes() {
	
		lifecoach_sc_map("trx_twitter", array(
			"title" => esc_html__("Twitter", 'lifecoach'),
			"desc" => wp_kses_data( __("Insert twitter feed into post (page)", 'lifecoach') ),
			"decorate" => false,
			"container" => false,
			"params" => array(
				"user" => array(
					"title" => esc_html__("Twitter Username", 'lifecoach'),
					"desc" => wp_kses_data( __("Your username in the twitter account. If empty - get it from Theme Options.", 'lifecoach') ),
					"value" => "",
					"type" => "text"
				),
				"consumer_key" => array(
					"title" => esc_html__("Consumer Key", 'lifecoach'),
					"desc" => wp_kses_data( __("Consumer Key from the twitter account", 'lifecoach') ),
					"value" => "",
					"type" => "text"
				),
				"consumer_secret" => array(
					"title" => esc_html__("Consumer Secret", 'lifecoach'),
					"desc" => wp_kses_data( __("Consumer Secret from the twitter account", 'lifecoach') ),
					"value" => "",
					"type" => "text"
				),
				"token_key" => array(
					"title" => esc_html__("Token Key", 'lifecoach'),
					"desc" => wp_kses_data( __("Token Key from the twitter account", 'lifecoach') ),
					"value" => "",
					"type" => "text"
				),
				"token_secret" => array(
					"title" => esc_html__("Token Secret", 'lifecoach'),
					"desc" => wp_kses_data( __("Token Secret from the twitter account", 'lifecoach') ),
					"value" => "",
					"type" => "text"
				),
				"count" => array(
					"title" => esc_html__("Tweets number", 'lifecoach'),
					"desc" => wp_kses_data( __("Tweets number to show", 'lifecoach') ),
					"divider" => true,
					"value" => 3,
					"max" => 20,
					"min" => 1,
				...
				"controls" => array(
					"title" => esc_html__("Show arrows", 'lifecoach'),
					"desc" => wp_kses_data( __("Show control buttons", 'lifecoach') ),
					"value" => "yes",
					"type" => "switch",
					"options" => lifecoach_get_sc_param('yes_no')
				),
				"interval" => array(
					"title" => esc_html__("Tweets change interval", 'lifecoach'),
					"desc" => wp_kses_data( __("Tweets change interval (in milliseconds: 1000ms = 1s)", 'lifecoach') ),
					"value" => 7000,
					"step" => 500,
					"min" => 0,
					"type" => "spinner"
				...
				"align" => array(
					...
					"desc" => wp_kses_data( __("Alignment of the tweets block", 'lifecoach') ),
					"divider" => true,
					"value" => "",
					"type" => "checklist",
					"dir" => "horizontal",
...
-------------------------------------------------------------------- */
if ( !function_exists( 'lifecoach_sc_twitter_reg_shortcodes_vc' ) ) {
	//add_action('lifecoach_action_shortcodes_list_vc', 'lifecoach_sc_twitter_reg_shortcodes_vc');
	function lifecoach_sc_twitter_reg_shortcodes_vc() {
	
		vc_map( array(
			"base" => "trx_twitter",
			"name" => esc_html__("Twitter", 'lifecoach'),
			"description" => wp_kses_data( __("Insert twitter feed into post (page)", 'lifecoach') ),
			...
			"params" => array(
				array(
					...
					"description" => wp_kses_data( __("Your username in the twitter account. If empty - get it from Theme Options.", 'lifecoach') ),
					"class" => "",
					"value" => "",
					"type" => "textfield"
				),
				array(
					...
					"description" => wp_kses_data( __("Consumer Key from the twitter account", 'lifecoach') ),
					"class" => "",
					"value" => "",
					"type" => "textfield"
				),
				array(
					...
					"description" => wp_kses_data( __("Consumer Secret from the twitter account", 'lifecoach') ),
					"class" => "",
					"value" => "",
					"type" => "textfield"
				),
				array(
					...
					"description" => wp_kses_data( __("Token Key from the twitter account", 'lifecoach') ),
					"class" => "",
					"value" => "",
					"type" => "textfield"
				),
				array(
					...
					"description" => wp_kses_data( __("Token Secret from the twitter account", 'lifecoach') ),
					"class" => "",
					"value" => "",
					"type" => "textfield"
				),
				array(
					...
					"description" => wp_kses_data( __("Number tweets to show", 'lifecoach') ),
					"class" => "",
					"divider" => true,
					"value" => 3,
					"type" => "textfield"
				...
				array(
					"param_name" => "controls",
					"heading" => esc_html__("Show arrows", 'lifecoach'),
					"description" => wp_kses_data( __("Show control buttons", 'lifecoach') ),
					"admin_label" => true,
					"class" => "",
				...
				array(
					"param_name" => "interval",
					"heading" => esc_html__("Tweets change interval", 'lifecoach'),
					"description" => wp_kses_data( __("Tweets change interval (in milliseconds: 1000ms = 1s)", 'lifecoach') ),
					"class" => "",
					"value" => "7000",
					"type" => "textfield"
				),
				array(
					...
					"description" => wp_kses_data( __("Alignment of the tweets block", 'lifecoach') ),
					"class" => "",
		...
		class WPBakeryShortCode_Trx_Twitter extends LIFECOACH_VC_ShortCodeSingle {}
	}
}
?>