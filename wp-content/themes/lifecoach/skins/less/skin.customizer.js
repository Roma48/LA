...
function lifecoach_skin_customizer(option, val) {
	...
	if (option == 'body_style') {
		...
		var params = LIFECOACH_STORAGE['co_add_params']!=undefined ? LIFECOACH_STORAGE['co_add_params'] : {};
		params[option] = val;
	...
	if (custom_style != '') {
		var styles = jQuery('#lifecoach-customizer-styles-'+option).length > 0 ? jQuery('#lifecoach-customizer-styles-'+option) : '';
		if (styles.length == 0)
			jQuery('head').append('<style id="lifecoach-customizer-styles-'+option+'" type="text/css">'+custom_style+'</style>');
		else
