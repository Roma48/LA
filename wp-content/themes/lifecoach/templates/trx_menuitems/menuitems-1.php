...
if ( !function_exists( 'lifecoach_template_menuitems_1_theme_setup' ) ) {
	add_action( 'lifecoach_action_before_init_theme', 'lifecoach_template_menuitems_1_theme_setup', 1 );
	function lifecoach_template_menuitems_1_theme_setup() {
		lifecoach_add_template(array(
			'layout' => 'menuitems-1',
			'template' => 'menuitems-1',
			'mode'   => 'menuitems',
			'title'  => esc_html__('MenuItems /Style 1/', 'lifecoach'),
			'thumb_title'  => esc_html__('Avatar (small)', 'lifecoach'),
			'w'		 => 75,
			'h'		 => 75
		));
	}
...
// Template output
if ( !function_exists( 'lifecoach_template_menuitems_1_output' ) ) {
	function lifecoach_template_menuitems_1_output($post_options, $post_data) {
		...
		?>
			<div<?php echo !empty($post_options['tag_id']) ? ' id="'.esc_attr($post_options['tag_id']).'"' : ''; ?> 
				...
				<div class="sc_menuitem_description"><?php echo trim(lifecoach_strshort($post_data['post_excerpt'], isset($post_options['descr']) ? $post_options['descr'] : lifecoach_get_custom_option('post_excerpt_maxlength_masonry'))) ?></div>
