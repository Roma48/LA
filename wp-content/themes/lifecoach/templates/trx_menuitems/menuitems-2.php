...
if ( !function_exists( 'lifecoach_template_menuitems_2_theme_setup' ) ) {
	add_action( 'lifecoach_action_before_init_theme', 'lifecoach_template_menuitems_2_theme_setup', 1 );
	function lifecoach_template_menuitems_2_theme_setup() {
		lifecoach_add_template(array(
			'layout' => 'menuitems-2',
			'template' => 'menuitems-2',
			'mode'   => 'menuitems',
			'title'  => esc_html__('MenuItems /Style 2/', 'lifecoach'),
			'thumb_title'  => esc_html__('Fullwidth image (crop)', 'lifecoach'),
			'w'		 => 1170,	//1920,
			'h'		 => 659		//698
		));
	}
...
// Template output
if ( !function_exists( 'lifecoach_template_menuitems_2_output' ) ) {
	function lifecoach_template_menuitems_2_output($post_options, $post_data) {
			...
			<div<?php echo !empty($post_options['tag_id']) ? ' id="'.esc_attr($post_options['tag_id']).'"' : ''; ?>
				...
				if (   ( $post_options['menuitem_calories'] != 'inherit'		&& strlen($post_options['menuitem_calories']) != 0 )
					...
					<div class="sc_menuitem_nutritions">
						...
						<ul class="sc_menuitem_nutritions_list">
							<?php
							if ( $post_options['menuitem_calories'] != 'inherit' && strlen($post_options['menuitem_calories']) != 0 )
								echo '<li>' . esc_html__('Calories: ', 'lifecoach') . '<span>' . trim($post_options['menuitem_calories']) . esc_html__('Kcal', 'lifecoach') . '</span></li>';
							if ( $post_options['menuitem_cholesterol'] != 'inherit' && strlen($post_options['menuitem_cholesterol']) != 0 ) {
								echo '<li>' . esc_html__('Cholesterol: ', 'lifecoach') . '<span>' . trim($post_options['menuitem_cholesterol']) . esc_html__('mg', 'lifecoach') . '</span></li>';
							}
							if ( $post_options['menuitem_fiber'] != 'inherit' && strlen($post_options['menuitem_fiber']) != 0 ) {
								echo '<li>' . esc_html__('Fiber: ', 'lifecoach') . '<span>' . trim($post_options['menuitem_fiber']) . esc_html__('g', 'lifecoach') . '</span></li>';
							}
							if ( $post_options['menuitem_sodium'] != 'inherit' && strlen($post_options['menuitem_sodium']) != 0 ) {
								echo '<li>' . esc_html__('Sodium: ', 'lifecoach') . '<span>' . trim($post_options['menuitem_sodium']) . esc_html__('mg', 'lifecoach') . '</span></li>';
							}
							if ( $post_options['menuitem_carbohydrates'] != 'inherit' && strlen($post_options['menuitem_carbohydrates']) != 0 ) {
								echo '<li>' . esc_html__('Carbohydrates: ', 'lifecoach') . '<span>' . trim($post_options['menuitem_carbohydrates']) . esc_html__('g', 'lifecoach') . '</span></li>';
							} 
							if ( $post_options['menuitem_fat'] != 'inherit' && strlen($post_options['menuitem_fat']) != 0 ) { 
								echo '<li>' . esc_html__('Fat: ', 'lifecoach') . '<span>' . trim($post_options['menuitem_fat']) . esc_html__('g', 'lifecoach') . '</span></li>';
							}
							if ( $post_options['menuitem_protein'] != 'inherit' && strlen($post_options['menuitem_protein']) != 0 ) {
								echo '<li>' . esc_html__('Protein: ', 'lifecoach') . '<span>' . trim($post_options['menuitem_protein']) . esc_html__('g', 'lifecoach') . '</span></li>';
							}
							?>
						</ul>
						<div class="cL"></div>
				...
				<div class="sc_menuitem_more">
					<?php if ( $post_options['menuitem_product'] != 'inherit' ) { ?>
						<a class="sc_button sc_button_square sc_button_style_filled sc_button_size_small margin_right_tiny" href="<?php echo get_permalink($post_options['menuitem_product']); ?>"><?php esc_html_e('ORDER', 'lifecoach'); ?></a>
					<?php } ?>
					<a class="sc_button sc_button_square sc_button_style_filled sc_button_size_small" href="<?php echo esc_url($post_data['post_comments_link']); ?>"><?php esc_html_e('POST COMMENT', 'lifecoach'); ?></a>
					<div class="cL"></div>
				</div>
