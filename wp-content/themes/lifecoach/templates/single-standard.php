...
if ( !function_exists( 'lifecoach_template_single_standard_theme_setup' ) ) {
	add_action( 'lifecoach_action_before_init_theme', 'lifecoach_template_single_standard_theme_setup', 1 );
	function lifecoach_template_single_standard_theme_setup() {
		lifecoach_add_template(array(
			'layout' => 'single-standard',
			'mode'   => 'single',
			'need_content' => true,
			'need_terms' => true,
			'title'  => esc_html__('Single standard', 'lifecoach'),
			'thumb_title'  => esc_html__('Fullwidth i mage (crop)', 'lifecoach'),
			'w'		 => 1170,
			'h'		 => 740
		));
	}
...
// Template output
if ( !function_exists( 'lifecoach_template_single_standard_output' ) ) {
	function lifecoach_template_single_standard_output($post_options, $post_data) {
        ...
        if ($post_data['post_type'] == 'services') {
            ?>
            <h5 itemprop="<?php echo (float) $avg_author > 0 || (float) $avg_users > 0 ? 'itemReviewed' : 'headline'; ?>" class="post_title entry-title"><?php echo trim($post_data['post_title']); ?></h5>
        <?php
        }
