...
$match_date = 	!empty($post_meta['match_date']) && !lifecoach_param_is_inherit($post_meta['match_date']) ? date("d M", strtotime($post_meta['match_date'])) : '';	
$match_time = 	!empty($post_meta['match_time']) && !lifecoach_param_is_inherit($post_meta['match_time']) ? $post_meta['match_time'] : '';	
$match_score = 	!empty($post_meta['match_score']) && !lifecoach_param_is_inherit($post_meta['match_score']) ? $post_meta['match_score'] : '0-0';
$match_passed = false;
...
if (!empty($first_player) && $first_player!='none' && ($first_player_obj = (intval($first_player) > 0 ? get_post($first_player, OBJECT) : get_page_by_title($first_player, OBJECT, 'players'))) != null) {
	$first_player_post_meta = get_post_meta($first_player_obj->ID, lifecoach_storage_get('options_prefix') . '_post_options', true);
	...
	$first_player_photo = lifecoach_get_resized_image_tag($first_player_photo, $thumb_sizes['w'], $thumb_sizes['h']);
}
...
if (!empty($second_player) && $second_player!='none' && ($second_player_obj = (intval($second_player) > 0 ? get_post($second_player, OBJECT) : get_page_by_title($second_player, OBJECT, 'players'))) != null) {
	$second_player_post_meta = get_post_meta($second_player_obj->ID, lifecoach_storage_get('options_prefix') . '_post_options', true);
	...
	$second_player_photo = lifecoach_get_resized_image_tag($second_player_photo, $thumb_sizes['w'], $thumb_sizes['h']);
}
