...
if ( !function_exists( 'lifecoach_template_news_excerpt_theme_setup' ) ) {
	add_action( 'lifecoach_action_before_init_theme', 'lifecoach_template_news_excerpt_theme_setup', 1 );
	function lifecoach_template_news_excerpt_theme_setup() {
		lifecoach_add_template(array(
			'layout' => 'news-excerpt',
			'template' => 'news-excerpt',
			'mode'   => 'news',
			'title'  => esc_html__('Recent News /Style Excerpt/', 'lifecoach'),
...
// Template output
if ( !function_exists( 'lifecoach_template_news_excerpt_output' ) ) {
	function lifecoach_template_news_excerpt_output($post_options, $post_data) {
						...
						if ($post_data['post_excerpt']) {
							echo in_array($post_data['post_format'], array('quote', 'link', 'chat', 'aside', 'status')) 
									...
									: wpautop(lifecoach_strshort($post_data['post_excerpt'], isset($post_options['descr']) 
																								? $post_options['descr'] 
																								: lifecoach_get_custom_option('post_excerpt_maxlength')
																)
											);
						}
					}
