...
if ( !function_exists( 'lifecoach_template_news_magazine_theme_setup' ) ) {
	add_action( 'lifecoach_action_before_init_theme', 'lifecoach_template_news_magazine_theme_setup', 1 );
	function lifecoach_template_news_magazine_theme_setup() {
		lifecoach_add_template(array(
			'layout' => 'news-magazine',
			'template' => 'news-magazine',
			'mode'   => 'news',
			'title'  => esc_html__('Recent News /Style Magazine/', 'lifecoach'),
...
// Template output
if ( !function_exists( 'lifecoach_template_news_magazine_output' ) ) {
	function lifecoach_template_news_magazine_output($post_options, $post_data) {
		...
		?><article id="post-<?php echo esc_html($post_data['post_id']); ?>" 
			...
			if ($number <= $featured) {
					...
					if ($post_data['post_protected']) {
						...
						if ($post_data['post_excerpt']) {
							echo in_array($post_data['post_format'], array('quote', 'link', 'chat', 'aside', 'status')) 
									...
									: wpautop(lifecoach_strshort($post_data['post_excerpt'], isset($post_options['descr']) 
																								? $post_options['descr'] 
				...
				<div class="post_footer entry-footer">
					...
					<?php
					if ( in_array( $post_data['post_type'], array( 'post', 'attachment' ) ) ) {
						$post_options['counters'] = 'views,comments,edit,captions';		//lifecoach_get_theme_option('blog_counters');
