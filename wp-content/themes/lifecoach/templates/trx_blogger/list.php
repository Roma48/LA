...
if ( !function_exists( 'lifecoach_template_list_theme_setup' ) ) {
	add_action( 'lifecoach_action_before_init_theme', 'lifecoach_template_list_theme_setup', 1 );
	function lifecoach_template_list_theme_setup() {
		lifecoach_add_template(array(
			'layout' => 'list',
			'mode'   => 'blogger',
			'need_columns' => true,
			'title'  => esc_html__('Blogger layout: List', 'lifecoach')
			));
	}
}

// Template output
if ( !function_exists( 'lifecoach_template_list_output' ) ) {
	function lifecoach_template_list_output($post_options, $post_data) {
