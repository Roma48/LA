...
if ( !function_exists( 'lifecoach_template_team_3_theme_setup' ) ) {
	add_action( 'lifecoach_action_before_init_theme', 'lifecoach_template_team_3_theme_setup', 1 );
	function lifecoach_template_team_3_theme_setup() {
		lifecoach_add_template(array(
			'layout' => 'team-3',
			'template' => 'team-3',
			'mode'   => 'team',
			/*'container_classes' => 'sc_slider_nopagination sc_slider_controls sc_slider_controls_side',*/
			'title'  => esc_html__('Team /Style 3/', 'lifecoach'),
...
// Template output
if ( !function_exists( 'lifecoach_template_team_3_output' ) ) {
	function lifecoach_template_team_3_output($post_options, $post_data) {
		$show_title = true;
		...
		?>
			<div<?php echo !empty($post_options['tag_id']) ? ' id="'.esc_attr($post_options['tag_id']).'"' : ''; ?>
				...
				<div class="sc_team_item_info">
					<h6 class="sc_team_item_title"><?php echo (!empty($post_options['link']) ? '<a href="'.esc_url($post_options['link']).'">' : '') . ($post_data['post_title']) . (!empty($post_options['link']) ? '</a>' : ''); ?></h6>
