...
if ( !function_exists( 'lifecoach_template_single_matches_theme_setup' ) ) {
	add_action( 'lifecoach_action_before_init_theme', 'lifecoach_template_single_matches_theme_setup', 1 );
	function lifecoach_template_single_matches_theme_setup() {
		lifecoach_add_template(array(
			'layout' => 'single-matches',
			'mode'   => 'matches',
			'need_content' => true,
			'need_terms' => true,
			'title'  => esc_html__('Single matches', 'lifecoach'),
...
// Template output
if ( !function_exists( 'lifecoach_template_single_matches_output' ) ) {
	function lifecoach_template_single_matches_output($post_options, $post_data) {
		...
		if (!$post_data['post_protected']) {
			...
			get_template_part(lifecoach_get_file_slug('templates/trx_matches/_parts/match-info.php'));
		}
		...
		?>
			<div class="match_preview"><a href="<?php echo esc_url($match_preview); ?>"><?php echo esc_html__("Go to the announcement &#8594;", 'lifecoach'); ?></a></div>
		<?php
		} 
		...
		if (!$sidebar_present) lifecoach_close_wrapper();	// .post_item
