...
// Theme init
if (!function_exists('lifecoach_widget_recent_news_theme_setup')) {
	add_action( 'lifecoach_action_before_init_theme', 'lifecoach_widget_recent_news_theme_setup', 1 );
	function lifecoach_widget_recent_news_theme_setup() {

		// Register shortcodes in the shortcodes list
		//add_action('lifecoach_action_shortcodes_list',	'lifecoach_widget_recent_news_reg_shortcodes');
		if (function_exists('lifecoach_exists_visual_composer') && lifecoach_exists_visual_composer())
			add_action('lifecoach_action_shortcodes_list_vc','lifecoach_widget_recent_news_reg_shortcodes_vc');
	}
}

// Load widget
if (!function_exists('lifecoach_widget_recent_news_load')) {
	add_action( 'widgets_init', 'lifecoach_widget_recent_news_load' );
	function lifecoach_widget_recent_news_load() {
		register_widget('lifecoach_widget_recent_news');
	}
}


// Widget Class
class lifecoach_widget_recent_news extends WP_Widget {

	function __construct() {
		$widget_ops = array('classname' => 'widget_recent_news', 'description' => esc_html__('Show recent news in many styles', 'lifecoach'));
		parent::__construct( 'lifecoach_widget_recent_news', esc_html__('ThemeREX - Recent News', 'lifecoach'), $widget_ops );
	}
	...
	function widget($args, $instance) {
		...
		$output = lifecoach_sc_recent_news( array(
			'from_widget'		=> true,
	...
	function form($instance) {
		...
		$news_styles = lifecoach_get_list_templates('news');
		?>

		<p>
			<label for="<?php echo esc_attr($this->get_field_id('widget_title')); ?>"><?php esc_html_e('Widget title:', 'lifecoach'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('title')); ?>"><?php esc_html_e('Block title:', 'lifecoach'); ?></label>
			<input id="<?php echo esc_attr($this->get_field_id('title')); ?>" name="<?php echo esc_attr($this->get_field_name('title')); ?>" value="<?php echo esc_attr($title); ?>" class="widgets_param_fullwidth" />
		</p>

		<p>
			<label for="<?php echo esc_attr($this->get_field_id('subtitle')); ?>"><?php esc_html_e('Block subtitle:', 'lifecoach'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('style')); ?>"><?php esc_html_e('Style:', 'lifecoach'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('count')); ?>"><?php esc_html_e('Number of posts to be displayed:', 'lifecoach'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('featured')); ?>"><?php esc_html_e('Number of featured posts:', 'lifecoach'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('columns')); ?>"><?php esc_html_e('Number of columns:', 'lifecoach'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('show_categories')); ?>_1"><?php esc_html_e('Show categories:', 'lifecoach'); ?></label><br />
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_categories')); ?>_1"><?php esc_html_e('Show', 'lifecoach'); ?></label>
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_categories')); ?>_0"><?php esc_html_e('Hide', 'lifecoach'); ?></label>
		</p>

		<p>
			<label for="<?php echo esc_attr($this->get_field_id('show_counters')); ?>_1"><?php esc_html_e('Show counters:', 'lifecoach'); ?></label><br />
...
*/
if ( !function_exists( 'lifecoach_sc_recent_news' ) ) {
	function lifecoach_sc_recent_news($atts, $content=null){	
		...
		// Get categories list
		if (lifecoach_param_is_on($show_categories)) {
			if ( !lifecoach_storage_isset('categories_'.$category) ) {
				lifecoach_storage_set('categories_'.$category, get_categories(array(
					'orderby' => 'name',
					'parent' => $category
					))
				);
		...
		if (empty($from_widget)) {
			$widget_args = lifecoach_prepare_widgets_args(lifecoach_storage_get('widgets_args'), $id ? $id.'_widget' : 'widget_recent_news', 'widget_recent_news');
			$output .= '<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
							. ' class="widget_area sc_recent_news_wrap' 
								. (lifecoach_exists_visual_composer() ? ' vc_recent_news wpb_content_element' : '') 
		...
		// Header
		if ( !empty($title) || !empty($subtitle) || (lifecoach_param_is_on($show_categories) && !lifecoach_storage_empty('categories_'.$category)) ) {
			$output	.= '<div class="sc_recent_news_header'.(lifecoach_param_is_on($show_categories) && !lifecoach_storage_empty('categories_'.$category) ? ' sc_recent_news_header_split' : '').'">'
			...
			// Categories list
			$cat_list = lifecoach_storage_get('categories_'.$category);
			if (lifecoach_param_is_on($show_categories) && !empty($cat_list)) {
				...
				if (is_array($cat_list) && count($cat_list) > 0) {
					$output .= '<a href="' . esc_url( $category == 0 
						...
						: get_category_link($category) ) . '" class="sc_recent_news_header_category_item">'.esc_html__('All News', 'lifecoach').'</span>';
					$number = 0;
					$number_max = 3;
					foreach ($cat_list as $cat) {
						$number++;
						if ($number == $number_max)
							$output .= '<span class="sc_recent_news_header_category_item sc_recent_news_header_category_item_more">'.esc_html__('More', 'lifecoach')
		...
		$args = lifecoach_query_add_posts_and_cats($args, $ids, $post_type, $category, $taxonomy);
		...
		return apply_filters('lifecoach_shortcode_output', $output, 'trx_recent_news', $atts, $content);
	}
	lifecoach_require_shortcode("trx_recent_news", "lifecoach_sc_recent_news");
}


// Add [trx_recent_news] in the VC shortcodes list
if (!function_exists('lifecoach_widget_recent_news_reg_shortcodes_vc')) {
	//add_action('lifecoach_action_shortcodes_list_vc','lifecoach_widget_recent_news_reg_shortcodes_vc');
	function lifecoach_widget_recent_news_reg_shortcodes_vc() {
		...
		vc_map( array(
				...
				"name" => esc_html__("Recent News", 'lifecoach'),
				"description" => wp_kses_data( __("Insert recent news list", 'lifecoach') ),
				...
				"params" => array(
					array(
						"param_name" => "style",
						"heading" => esc_html__("List style", 'lifecoach'),
						"description" => wp_kses_data( __("Select style to display news list", 'lifecoach') ),
						"class" => "",
						"admin_label" => true,
						"std" => 'news-magazine',
					...
					array(
						...
						"description" => wp_kses_data( __("Title for the widget (fill this field only if you want to use shortcode as widget)", 'lifecoach') ),
						"admin_label" => true,
						"class" => "",
						"value" => "",
						"type" => "textfield"
					...
					array(
						...
						"heading" => esc_html__("Show categories", 'lifecoach'),
						"description" => wp_kses_data( __("Show categories in the shortcode's header", 'lifecoach') ),
						"class" => "",
					...
					array(
						...
						"heading" => esc_html__("Show counters", 'lifecoach'),
						"description" => wp_kses_data( __("Show counters (comments and views) in the shortcode's footer", 'lifecoach') ),
						'dependency' => array(
							'element' => 'style',
							'value' => 'news-magazine'
						),
					...
					array(
						...
						"description" => wp_kses_data( __("Select category to show news. If empty - select news from any category or from IDs list", 'lifecoach') ),
						"group" => esc_html__('Query', 'lifecoach'),
						"class" => "",
					...
					array(
						"param_name" => "count",
						"heading" => esc_html__("Total posts", 'lifecoach'),
					...
					array(
						"param_name" => "featured",
						"heading" => esc_html__("Featured posts", 'lifecoach'),
						"description" => wp_kses_data( __("How many posts will be displayed as featured?", 'lifecoach') ),
						"admin_label" => true,
						"group" => esc_html__('Query', 'lifecoach'),
						'dependency' => array(
							'element' => 'style',
					...
					array(
						...
						"description" => wp_kses_data( __("How many columns use to show news list", 'lifecoach') ),
						"group" => esc_html__('Query', 'lifecoach'),
						"admin_label" => true,
						'dependency' => array(
							'element' => 'style',
					...
					array(
						...
						"group" => esc_html__('Query', 'lifecoach'),
						"class" => "",
						"value" => array_flip(lifecoach_get_list_sortings()),
						"type" => "dropdown"
					),
					array(
						"param_name" => "order",
						...
						"group" => esc_html__('Query', 'lifecoach'),
						"class" => "",
						"value" => array_flip(lifecoach_get_list_orderings()),
						"type" => "dropdown"
					),
					array(
						"param_name" => "ids",
						"heading" => esc_html__("List IDs", 'lifecoach'),
						"description" => wp_kses_data( __("Comma separated list of post's ID. If set - parameters above (category, count, order, etc.) are ignored!", 'lifecoach') ),
						"group" => esc_html__('Query', 'lifecoach'),
						"class" => "",
						"value" => "",
						"type" => "textfield"
