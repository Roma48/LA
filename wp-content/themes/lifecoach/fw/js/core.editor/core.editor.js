jQuery(document).ready(function () {
	...
	jQuery('#frontend_editor_icon_edit').on('click', function (e) {
		...
		jQuery('#frontend_editor_button_cancel').val(LIFECOACH_STORAGE['strings']['editor_caption_cancel']);
	...
	jQuery('#frontend_editor_button_cancel').on('click', function (e) {
		"use strict";
		if (jQuery(this).val() == LIFECOACH_STORAGE['strings']['editor_caption_close'])
	...
	jQuery('#frontend_editor_button_save').on('click', function (e) {
		...
		jQuery.post(LIFECOACH_STORAGE['ajax_url'], data, function(response) {
			...
			if (rez.error == '') {
				lifecoach_message_success('', LIFECOACH_STORAGE['strings']['editor_save_success']);
				jQuery('#frontend_editor_button_cancel').val(LIFECOACH_STORAGE['strings']['editor_caption_close']);
			} else {
				lifecoach_message_warning(rez.error, LIFECOACH_STORAGE['strings']['editor_save_error']);
			}
		});
		e.preventDefault();
		return false;
	...
	jQuery('#frontend_editor_icon_delete').on('click', function (e) {
		"use strict";
		lifecoach_message_confirm(LIFECOACH_STORAGE['strings']['editor_delete_post'], LIFECOACH_STORAGE['strings']['editor_delete_post_header'], function(btn) {
			"use strict";
			if (btn != 1) return;
			var data = {
			...
			jQuery.post(LIFECOACH_STORAGE['ajax_url'], data, function(response) {
				...
				if (rez.error == '') {
					lifecoach_message_success('', LIFECOACH_STORAGE['strings']['editor_delete_success']);
					setTimeout(function() { 
						window.location.href = LIFECOACH_STORAGE['site_url'];
						}, 1000);
				} else {
					lifecoach_message_warning(rez.error, LIFECOACH_STORAGE['strings']['editor_delete_error']);
				}
			});
