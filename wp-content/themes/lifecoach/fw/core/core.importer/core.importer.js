...
jQuery(document).ready(function(){
	...
	jQuery('.trx_importer_section').on('click', '.trx_buttons input[type="button"]', function() {
		...
		LIFECOACH_STORAGE['importer_error_messages'] = '';
		LIFECOACH_STORAGE['importer_ignore_errors'] = true;
		lifecoach_importer_do_action(steps, 0);
	});
});

// Call specified action (step)
function lifecoach_importer_do_action(steps, idx) {
	"use strict";
	...
	// AJAX query params
	var data = {
		ajax_nonce: LIFECOACH_STORAGE['ajax_nonce'],
		action: 'lifecoach_importer_start_import',
	...
	jQuery.post(LIFECOACH_STORAGE['ajax_url'], data, function(response) {
		...
		if (rez.error === '' || LIFECOACH_STORAGE['importer_ignore_errors']) {
			if (rez.error !== '') 
				LIFECOACH_STORAGE['importer_error_messages'] += '<p class="error_message">' + rez.error + '</p>';
			...
			// Do next action
			if (idx < steps.length) {
				lifecoach_importer_do_action(steps, idx);
			} else {
				if (LIFECOACH_STORAGE['importer_error_messages']) {
					jQuery('#trx_importer_progress').removeClass('notice-info').addClass('notice-error').append('<h4>' + LIFECOACH_STORAGE['importer_error_msg'] + '</h4>' + LIFECOACH_STORAGE['importer_error_messages']);
				} else {
