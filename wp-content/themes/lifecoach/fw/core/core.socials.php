<?php
/**
 * LifeCoach Framework: social networks
 *
 * @package	lifecoach
 * @since	lifecoach 1.0
 */
...
// Theme init
if (!function_exists('lifecoach_socials_theme_setup')) {
	add_action( 'lifecoach_action_before_init_theme', 'lifecoach_socials_theme_setup' );
	function lifecoach_socials_theme_setup() {

		if ( !is_admin() ) {
			// Add og:image meta tag for facebook
			add_action( 'wp_head', 					'lifecoach_facebook_og_tags', 5 );
		}
...
lifecoach_storage_set('share_links', array(
...
//	'facebook' =>		'http://www.facebook.com/sharer.php?s=100&p[url]={link}&p[title]={title}&p[summary]={descr}&p[images][0]={image}',
	...
	'postila' =>		'http://postila.ru/publish/?url={link}&agregator=lifecoach',
...
if (!function_exists('lifecoach_add_share_link')) {
	function lifecoach_add_share_link($soc, $url='') {
		if (!is_array($soc)) $soc = array($soc => $url);
		lifecoach_storage_set('share_links', array_merge( lifecoach_storage_get('share_links'), $soc ) );
	}
}

// Return (and show) share social links
if (!function_exists('lifecoach_show_share_links')) {
	function lifecoach_show_share_links($args) {
		if ( lifecoach_get_custom_option('show_share')=='hide' ) return '';
		...
		$args = array_merge(array(
			...
			'style' => lifecoach_get_theme_setting('socials_type')=='images' ? 'bg' : 'icons',	// style for show icons: icons|images|bg
			'type' => 'block',					// share block type: list|block|drop
			'popup' => true,					// open share url in new window or in popup window
			'counters' => lifecoach_get_custom_option('show_share_counters')=='yes',	// show share counters
			'direction' => lifecoach_get_custom_option('show_share'),				// share block direction
			'caption' => lifecoach_get_custom_option('share_caption'),				// share block caption
			'share' => lifecoach_get_theme_option('share_buttons'),					// list of allowed socials
		...
		if (is_array($args['share']) && count($args['share']) > 0) {
			foreach ($args['share'] as $soc) {
				$icon = $args['style']=='icons' || lifecoach_strpos($soc['icon'], $upload_url)!==false ? $soc['icon'] : lifecoach_get_socials_url(basename($soc['icon']));
				if ($args['style'] == 'icons') {
				...
				$url = $soc['url'];
				if (empty($url)) $url = lifecoach_storage_get_array('share_links', $sn);
				...
				$output .= '<div class="sc_socials_item' . (!empty($args['popup']) ? ' social_item_popup' : '') . '">'
					...
					. ($args['type']=='drop' ? '<i>' . trim(lifecoach_strtoproper($sn)) . '</i>' : '')
					. '</div>';
			}
		}
		$output .= '</div>';
...
if (!function_exists('lifecoach_prepare_socials')) {
	function lifecoach_prepare_socials($list, $style='') {
		if (empty($style)) $style = lifecoach_get_theme_setting('socials_type')=='images' ? 'bg' : 'icons';
		$output = '';
		$upload_info = wp_upload_dir();
		$upload_url = $upload_info['baseurl'];
		if (is_array($list) && count($list) > 0) {
			foreach ($list as $soc) {
				...
				$icon = $style=='icons' || lifecoach_strpos($soc['icon'], $upload_url)!==false ? $soc['icon'] : lifecoach_get_socials_url(basename($soc['icon']));
				if ($style == 'icons') {
					$parts = explode('-', $soc['icon'], 2);
					$sn = isset($parts[1]) ? $parts[1] : $parts[0];
				} else {
...
if (!function_exists('lifecoach_get_twitter_data')) {
	function lifecoach_get_twitter_data($cfg) {
...
if (!function_exists('lifecoach_prepare_twitter_text')) {
	function lifecoach_prepare_twitter_text($tweet) {
		$text = $tweet['text'];
...
if (!function_exists('lifecoach_get_twitter_followers')) {
	function lifecoach_get_twitter_followers($cfg) {
		$data = lifecoach_get_twitter_data($cfg); 
...
if (!function_exists('lifecoach_get_facebook_likes')) {
	function lifecoach_get_facebook_likes($account) {
		$fb = get_transient("facebooklikes");
		if ($fb !== false) return $fb;
		$fb = '?';
		$url = esc_url(lifecoach_get_protocol().'http://graph.facebook.com/'.($account));
		$headers = get_headers($url);
		if (lifecoach_strpos($headers[0], '200')) {
			$json = lifecoach_fgc($url);
...
if (!function_exists('lifecoach_facebook_og_tags')) {
	function lifecoach_facebook_og_tags() {
		global $post;
		if ( !is_singular() || lifecoach_storage_get('blog_streampage')) return;
...
if (!function_exists('lifecoach_get_feedburner_counter')) {
	function lifecoach_get_feedburner_counter($account) {
		$rss = get_transient("feedburnercounter");
		if ($rss !== false) return $rss;
		$rss = '?';
		$url = esc_url(lifecoach_get_protocol().'://feedburner.google.com/api/awareness/1.0/GetFeedData?uri='.($account));
		$headers = get_headers($url);
		if (lifecoach_strpos($headers[0], '200')) {
			$xml = lifecoach_fgc($url);
